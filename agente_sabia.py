...
async def processar_consulta(pergunta: str, client, session, system_prompt, tools):
    ...
    # Chamada única para o Sabiá decidir a busca
    ...
    if msg_ia.tool_calls:
        ...
        # OTIMIZAÇÃO: Processamos apenas a primeira chamada de ferramenta consolidada
        tool_call = msg_ia.tool_calls[0]
        args = json.loads(tool_call.function.arguments)
        busca_consolidada = args["interesse"] # O prompt agora pede um texto fluido
        
        print(f"🔍 Buscando: {busca_consolidada}", file=sys.stderr)
        
        res_mcp = await session.call_tool("buscar_materias_unb", arguments={"interesse": busca_consolidada})
        
        mensagens.append({
            "tool_call_id": tool_call.id,
            "role": "tool",
            "name": "buscar_materias_unb",
            "content": res_mcp.content[0].text
        })

        final_response = client.chat.completions.create(model="sabia-4", messages=mensagens)
